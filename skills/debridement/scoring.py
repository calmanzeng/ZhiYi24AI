...
class SkillScorer:
    ...
    def extract_features(self, landmarks):
        ...
        if "left_wrist" in self.landmark_history:
            lt = self.landmark_history["left_wrist"][-5:]
            if len(lt)>=2:
                sp=[np.sqrt((lt[i][0]-lt[i-1][0])**2+(lt[i][1]-lt[i-1][1])**2) for i in range(1,len(lt))]
                f["wrist_speed"]=float(np.mean(sp)) if sp else 0
        if "left_wrist" in self.landmark_history and "right_wrist" in self.landmark_history:
    ...
    def compute_metrics(self, features):
        ...
        m["scope"]=round(90 if 0.1<=ms<=0.5 else (70 if ms<0.1 else 60),1)
        ea=features.get("elbow_angle",90)
        p=features.get("hand_symmetry",0.7)*100
        p=p*0.7+max(0,100-abs(ea-90)*1.5)*0.3
        m["precision"]=round(min(100,p),1)
        sp=features.get("wrist_speed",1)
        m["speed"]=round(85 if sp<1.5 else (70 if sp<2.5 else 50),1)
    ...
    def get_feedback(self, metrics, features):
        ...
        if features.get("hand_symmetry",0.7)<0.6: fb.append({"type":"warning","message":"双手配合不协调","severity":"warning","metric":"coordination"})
        if features.get("movement_scope",0.2)>0.5: fb.append({"type":"info","message":"消毒/清创范围过大","severity":"info","metric":"scope"})
        if features.get("wrist_speed",1)>2.5: fb.append({"type":"warning","message":"操作速度过快","severity":"warning","metric":"speed"})
